...
export class GridstackComponent implements OnInit, AfterContentInit, OnDestroy {
  ...
  @Input() public set options(val: GridStackOptions) {
    this._options = val;
  }
  /** return the current running options */
  public get options(): GridStackOptions {
    return this._grid?.opts || this._options || {};
  }
  ...
  private hookEvents(grid?: GridStack) {
    ...
    grid
      .on('added', (event: Event, nodes: GridStackNode[]) =>
        this.ngZone.run(() => {
          this.added.emit({ event, nodes });
          this.checkEmpty();
        })
      )
      .on('change', (event: Event, nodes: GridStackNode[]) =>
        this.ngZone.run(() => this.changeGS.emit({ event, nodes }))
      )
      .on('disable', (event: Event) =>
        this.ngZone.run(() => this.disable.emit({ event }))
      )
      .on('drag', (event: Event, el: GridItemHTMLElement) =>
        this.ngZone.run(() => this.drag.emit({ event, el }))
      )
      .on('dragstart', (event: Event, el: GridItemHTMLElement) =>
        this.ngZone.run(() => this.dragstart.emit({ event, el }))
      )
      .on('dragstop', (event: Event, el: GridItemHTMLElement) =>
        this.ngZone.run(() => this.dragstop.emit({ event, el }))
      )
      .on(
        'dropped',
        (event: Event, previousNode: GridStackNode, newNode: GridStackNode) =>
          this.ngZone.run(() =>
            this.dropped.emit({ event, previousNode, newNode })
          )
      )
      .on('enable', (event: Event) =>
        this.ngZone.run(() => this.enable.emit({ event }))
      )
      .on('removed', (event: Event, nodes: GridStackNode[]) =>
        this.ngZone.run(() => {
          this.removed.emit({ event, nodes });
          this.checkEmpty();
        })
      )
      .on('resize', (event: Event, el: GridItemHTMLElement) =>
        this.ngZone.run(() => this.resize.emit({ event, el }))
      )
      .on('resizestart', (event: Event, el: GridItemHTMLElement) =>
        this.ngZone.run(() => this.resizestart.emit({ event, el }))
      )
      .on('resizestop', (event: Event, el: GridItemHTMLElement) =>
        this.ngZone.run(() => this.resizestop.emit({ event, el }))
      );
  }

  /** called by GS when a new item needs to be created, which we do as a Angular component, or deleted (skip) */
  private _addRemoveCB(
    g: GridStack,
    w: GridStackWidget,
    add: boolean
